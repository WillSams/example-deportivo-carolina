...
jest.mock('../../../src/utils/responses.js', () => ({ dbGet: jest.fn(), dbQuery: jest.fn() }));
...
describe('Resolvers - Queries', () => {
  ...
  const playersQueryByTeamSpy = jest.spyOn(Player, 'queryByTeam').mockResolvedValue(Players);
  const gameQueryByTeamSpy = jest.spyOn(Game, 'queryByTeam').mockResolvedValue(Games);
  ...
  dbQuery.mockImplementation(({ query }) => {
    if (JSON.stringify(query) === JSON.stringify(Game.queryByTeam({ teamId: 'test-team-3' }))) {
      return new Promise((resolve, reject) => {
        resolve(Games);
      });
    }
    if (JSON.stringify(query) === JSON.stringify(Player.queryByTeam({ teamId: 'test-team-3' }))) {
      return new Promise((resolve, reject) => {
        resolve(Players);
      });
    }
